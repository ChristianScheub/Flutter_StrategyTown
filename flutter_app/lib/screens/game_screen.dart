...
import '../services/game/game_state_notifier.dart' show gameMapControllerProvider;
import '../services/game_service.dart' as app_service;  // Add prefix to local game service
import '../widgets/unified_action_panel.dart';
import '../widgets/game_map.dart';
import '../widgets/resource_panel.dart';
import '../widgets/turn_info_panel.dart';
                  ...
                  onSelected: (value) async {
                    ...
                    switch (value) {
                      ...
                      case 'load':
                        ...
                        break;
                      case 'autosave':
                        // Use the new SaveGameService for autosaving
                        ...
                        if (mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text(success ? 'Game quick-saved' : 'Failed to quick-save game')),
                          );
                        }
                        break;
                      case 'load_autosave':
                        // Find the quick save in save list
                        final saves = await SaveGameService.getSaveList();
                        final quickSave = saves.firstWhere(
                          (save) => save['name'] == 'Quick Save',
                          orElse: () => <String, dynamic>{},
                        );
                        
                        if (quickSave.isNotEmpty) {
                          final gameState = await SaveGameService.loadGame(quickSave['key']);
                          if (gameState != null) {
                            // Update the game state - use prefixed provider
                            ref.read(app_service.gameServiceProvider.notifier).loadGameState(gameState);
                            if (mounted) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(content: Text('Quick save loaded')),
                              );
                            }
                          } else if (mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(content: Text('Failed to load quick save')),
                            );
                          }
                        } else if (mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('No quick save found')),
                          );
                        }
                        break;
